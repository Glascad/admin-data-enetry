...
describe('System Set Tests', () => {
    ...
    it('Can update a system set', () => {
        ...
        // system options
        cy.wait(2000);
        cy.getDataCy`system-name`.find('input').invoke('val').should('match', /test.system/i);
        cy.getDataCy`system-set-name`.invoke('val').should('match', /test/i);
        ...
        // sill.shim_support
        // cy.getDataCy`SILL.SHIM_SUPPORT`.click({ force: true }).should('have.checked');
        // can toggle configuration on or off
        // cy.getDataCy`SILL.SHIM_SUPPORT`.click({ force: true }).should('not.have.checked');
