...
class BatchedWrapper extends Component {
    ...
    // updateState = label => (value, cb) => this.setState(log({
    //     ...this.state,
    //     [label]: value
    // }, cb));

    updateState = (...args) => console.log("UPDATING STATE", ...args) || this.setState(...args);

    mutationIds = {};

    ChildComponent = createInputComponent({
        queryData: {},
        state: this.state,
    ...
    componentDidUpdate = ({
        ...
        if (!shallowEquals(data, this.props.apollo.queryStatus.data)) {
            this.ChildComponent = createInputComponent({
                queryData: this.props.apollo.queryStatus.data,
                state: this.state,
                updateState: this.updateState,
            })
        }
