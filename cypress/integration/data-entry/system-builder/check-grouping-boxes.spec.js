describe('Testing  actions in system builder', () => {
    ...
    it('Can check grouping boxes for correct toggles and information', () => {
        ...
        cy.getDataCy`toggle-child-parts`.should('exist').should('have.class', 'empty');

        // adding an option disables the part toggle
        cy.getDataCy`add-option`.click();
        cy.getDataCy`toggle-child-parts`.should('have.class','disabled');
        // deleting option makes the part toggle clickable again
        cy.getDataCy`delete-child-add_option`.click();
        cy.getDataCy`toggle-child-parts`.should('not.have.class','disabled');
        
        // same with parts
        cy.getDataCy`toggle-child-parts`.click();
